...
class ExampleUnitTest {
    ...
    private lateinit var viewModelClass: HomeViewModelClass

   // val handle = SavedStateHandle(mapOf("id" to "123"))
    @Before
    fun setBeforeTest(){
        repo= mockk(name = "send complaint")
        room= mockk()
        locaiton= mockk()
        validator= mockk()

      // viewModelClass= HomeViewModelClass(repository = repo, userRepoComplint = room, fetcher = locaiton, validator = validator, mainDispatcher = testDispatcher, ioDispatcher = testDispatcher)
...
//    •	capture(slot)
    ...
    @Test
    fun sendComplaintTestViewModel()= runTest{
